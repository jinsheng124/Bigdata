...
class QueryInfo:
    ...
    def _delaydel(self, key, e_time, div = 100):
        ...
        while time.time() < e_time:
            if key not in self.lru_cache.cache:
                print(f"数据{key}已淘汰,子线程退出")
                return
            time.sleep(n_time)
...
class Query:
    ...
    def __call__(self, fun):
        # 初始化
        # query_info = QueryInfo(max_size=self.max_size)
...
@timeit
@Query(max_size=20000) # max_size表示最大缓存数据条数
